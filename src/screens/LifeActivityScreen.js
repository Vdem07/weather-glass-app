...
export default function LifeActivityScreen({ navigation, route }) {
  ...
  const getRealUVIndex = () => {
    // Используем встроенный UV из weather объекта
    if (weather && weather.uv_index !== undefined) {
      return weather.uv_index;
    }
    
    // Если UV не найден в параметрах маршрута, используем переданный uvIndex
    if (uvIndex !== undefined && uvIndex !== null) {
      return uvIndex;
    }
    
    // Fallback к приблизительному расчету если UV данные недоступны
    return calculateApproximateUV(weather);
  };

  // Примерный расчет UV на основе доступных данных
const calculateApproximateUV = (weather) => {
  const hour = new Date().getHours();
  const cloudiness = weather.clouds?.all || 0;
  
  let baseUV = 0;
  
  // Базовый UV по времени дня
  if (hour >= 10 && hour <= 16) {
    baseUV = 7; // Пик солнца
  } else if (hour >= 8 && hour < 10 || hour > 16 && hour <= 18) {
    baseUV = 4; // Утро/вечер
  } else {
    baseUV = 1; // Рано утром/поздно вечером
  }
  
  // Корректировка на облачность
  const cloudFactor = (100 - cloudiness) / 100;
  const adjustedUV = Math.round(baseUV * cloudFactor);
  
  return Math.max(0, Math.min(11, adjustedUV));
};
...
const getAllergyData = () => {
  ...
  let windPollenThreshold, windHighThreshold;
  ...
  switch (windUnit) {
    ...
    case 'km/h':
      ...
      windHighThreshold = 18;     // 5 м/с = 18 км/ч
      break;
    case 'mph':
      windPollenThreshold = 6.7;  // 3 м/с = 6.7 mph
      windHighThreshold = 11.2;   // 5 м/с = 11.2 mph
      break;
    default:
      windPollenThreshold = 3;
      windHighThreshold = 5;
  ...
  // Для расчета score используем исходную температуру в Цельсиях
  const tempCelsius = weather.main.temp;
  const tempPollenThreshold = 15; // Всегда в Цельсиях для консистентности
  
  // Факторы влияния на пыльцу (всегда используем исходные значения в стандартных единицах)
  const windSpeedMS = weather.wind.speed; // Исходная скорость ветра в м/с
  if (windSpeedMS > 3) pollenScore += 2; // Ветер разносит пыльцу
  if (humidity < 50) pollenScore += 1; // Сухой воздух
  if (weather.weather[0].main === 'Clear') pollenScore += 2; // Ясная погода
  if (tempCelsius > tempPollenThreshold) pollenScore += 1; // Теплая погода
  ...
  // Функция для определения рекомендации по ветру
  const getWindRecommendation = () => {
    if (windSpeed > windHighThreshold) {
      return 'Сильный ветер усиливает распространение аллергенов';
    } else if (windSpeed > windPollenThreshold) {
      return 'Умеренный ветер может разносить пыльцу';
    } else {
      return 'Слабый ветер благоприятен для аллергиков';
    }
  };

  // Функция для определения рекомендации по влажности
  const getHumidityRecommendation = () => {
    if (humidity < 40) {
      return 'Низкая влажность повышает концентрацию пыльцы';
    } else if (humidity > 80) {
      return 'Высокая влажность может способствовать росту плесени';
    } else {
      return 'Влажность в норме для аллергиков';
    }
  };

  // Функция для определения общей рекомендации
  const getGeneralRecommendation = () => {
    if (pollenScore >= 5) {
      return 'Крайне высокий риск - оставайтесь дома';
    } else if (pollenScore >= 4) {
      return 'Избегайте длительного пребывания на улице';
    } else if (pollenScore >= 2) {
      return 'Можно гулять, но будьте осторожны';
    } else {
      return 'Можно гулять без опасений';
    }
  };
  ...
  return {
    conditions: [
      ...
      },
      {
        ...
        color: windSpeed > windHighThreshold ? '#FF9800' : '#4CAF50',
        icon: 'flag-outline',
      },
      {
        title: 'Влажность',
        value: `${humidity}%`,
        color: humidity < 40 ? '#FF9800' : humidity > 80 ? '#FFC107' : '#4CAF50',
        icon: 'water-outline',
      },
      {
        title: 'Видимость',
    ...
    recommendations: [
      getGeneralRecommendation(), // Улучшенная общая рекомендация
      getWindRecommendation(), // Корректная рекомендация по ветру
      getHumidityRecommendation(), // Улучшенная рекомендация по влажности
...
const getDrivingData = () => {
  ...
  // Функция для определения рекомендации по ветру
  const getWindRecommendation = () => {
    if (windSpeed > windDangerousThreshold) {
      return 'Сильный ветер - держите руль крепче';
    } else if (windSpeed > windCautiousThreshold) {
      return 'Умеренный ветер - будьте осторожны';
    } else {
      return 'Ветровые условия нормальные';
    }
  };

  // Функция для определения рекомендации по видимости
  const getVisibilityRecommendation = () => {
    if (visibility < 1000) {
      return 'Очень плохая видимость - включите противотуманные фары';
    } else if (visibility < 5000) {
      return 'Ограниченная видимость - снизьте скорость';
    } else {
      return 'Видимость в норме';
    }
  };
  ...
  const getGeneralRecommendation = () => {
    if (drivingScore <= 1) {
      return 'Крайне опасные условия - избегайте поездок';
    } else if (drivingScore <= 2) {
      return 'Рекомендуется избегать поездок';
    } else if (drivingScore <= 3) {
      return 'Соблюдайте повышенную осторожность';
    } else {
      return 'Соблюдайте обычную осторожность';
    }
  };
  ...
  return {
    ...
    recommendations: [
      getGeneralRecommendation(), // Улучшенная общая рекомендация
      getVisibilityRecommendation(), // Улучшенная рекомендация по видимости
      getWindRecommendation(), // Корректная рекомендация по ветру
      isSnow ? 'Используйте зимнюю резину и цепи' : 
        isRain ? 'Снизьте скорость на мокрой дороге' : 
        isFog ? 'Туман - включите противотуманные фары' :
        'Дорожные условия благоприятные',
    ]
  };
};
...
const getFishingData = () => {
  ...
  // Функция для определения рекомендации по ветру
  const getWindRecommendation = () => {
    if (windSpeed >= windOptimalMin && windSpeed <= windOptimalMax) {
      return 'Легкий ветер создает рябь - хорошо для рыбалки';
    } else if (windSpeed > windGoodMax) {
      return 'Сильный ветер затруднит рыбалку';
    } else if (windSpeed > windOptimalMax && windSpeed <= windGoodMax) {
      return 'Умеренный ветер - приемлемо для рыбалки';
    } else {
      return 'Штиль - рыба может быть менее активна';
    }
  };
  ...
  return {
    ...
    recommendations: [
      ...
      getWindRecommendation(), // Используем функцию с правильной логикой
...
const getWaterRecreationData = () => {
  const windSpeed = convertWindSpeed(weather.wind.speed, windUnit);
  const uvIndex = getRealUVIndex();
  
  let recreationScore = 0;
  let recreationCondition = 'Неподходящие';
  let recreationColor = '#f44336';
  
  // Для расчета score используем исходную температуру в Цельсиях для консистентности
  const tempCelsius = weather.main.temp;
  const waterTempCelsius = tempCelsius - 2; // Примерное снижение температуры воды в Цельсиях
  
  // Пороговые значения всегда в Цельсиях для score
  const tempExcellentThresholdC = 25;
  const tempGoodThresholdC = 20;
  
  if (tempCelsius >= tempExcellentThresholdC) recreationScore += 2;
  else if (tempCelsius >= tempGoodThresholdC) recreationScore += 1;
  ...
  // Функции для определения цветов (всегда на основе Цельсиев)
  const getAirTempColor = () => {
    if (tempCelsius >= tempExcellentThresholdC) return '#4CAF50';
    else if (tempCelsius >= tempGoodThresholdC) return '#FFC107';
    else return '#FF9800';
  };

  const getWaterTempColor = () => {
    const waterTempExcellentThresholdC = 22;
    const waterTempGoodThresholdC = 18;
    
    if (waterTempCelsius >= waterTempExcellentThresholdC) return '#4CAF50';
    else if (waterTempCelsius >= waterTempGoodThresholdC) return '#FFC107';
    else return '#FF9800';
  };

  // Функция для определения рекомендации по ветру
  const getWindRecommendation = () => {
    if (windSpeed <= windCalmThreshold) {
      return 'Спокойные условия на воде';
    } else if (windSpeed <= windModerateThreshold) {
      return 'Легкий ветерок - комфортно';
    } else {
      return 'Ветрено - возможны волны и прохлада';
    }
  };

  // Функция для определения рекомендации по температуре воздуха
  const getAirTempRecommendation = () => {
    if (tempCelsius >= tempExcellentThresholdC) {
      return 'Отличная погода для купания и загара';
    } else if (tempCelsius >= tempGoodThresholdC) {
      return 'Прохладно, но можно загорать';
    } else if (tempCelsius >= 15) {
      return 'Прохладно - лучше активный отдых без купания';
    } else {
      return 'Слишком прохладно для водных процедур';
    }
  };

  // Функция для определения рекомендации по UV
  const getUVRecommendation = () => {
    if (uvIndex > 7) {
      return 'Очень высокий UV - обязательно используйте солнцезащитный крем SPF 30+';
    } else if (uvIndex > 5) {
      return 'Высокий UV - используйте солнцезащитный крем и избегайте солнца в полдень';
    } else if (uvIndex > 3) {
      return 'Умеренный UV - защита от солнца рекомендуется';
    } else {
      return 'Низкий UV - безопасно загорать';
    }
  };

  // Функция для дополнительной рекомендации
  const getAdditionalRecommendation = () => {
    const isHot = tempCelsius > 30;
    const isWindy = windSpeed > windModerateThreshold;
    const isHighUV = uvIndex > 5;
    
    if (isHot && isHighUV) {
      return 'Жарко и высокий UV - пейте больше воды и прячьтесь в тени';
    } else if (isWindy && tempCelsius < 20) {
      return 'Ветрено и прохладно - возьмите теплую одежду';
    } else if (isHot) {
      return 'Жарко - не забывайте пить достаточно воды';
    } else {
      return 'Приятного отдыха у воды!';
    }
  };

  // Конвертируем температуры для отображения
  const tempDisplay = convertTemperature(tempCelsius, tempUnit);
  const waterTempDisplay = convertTemperature(waterTempCelsius, tempUnit);
  ...
  return {
    conditions: [
      ...
      {
        title: 'Температура\nвоздуха',
        value: `${Math.round(tempDisplay)}${getTemperatureSymbol(tempUnit)}`,
    ...
    recommendations: [
      getAirTempRecommendation(), // Улучшенная рекомендация по температуре
      getWindRecommendation(), // Корректная рекомендация по ветру
      getUVRecommendation(), // Детальная рекомендация по UV
      getAdditionalRecommendation(), // Дополнительная contextual рекомендация
    ]
  };
};
...
const getGardeningData = () => {
  ...
  // Пороговые значения всегда в Цельсиях для score
  const tempOptimalMinC = 15;
  const tempOptimalMaxC = 28;
  const tempGoodMinC = 10;
  const tempGoodMaxC = 32;
  
  if (tempCelsius >= tempOptimalMinC && tempCelsius <= tempOptimalMaxC) gardeningScore += 2;
  else if (tempCelsius >= tempGoodMinC && tempCelsius <= tempGoodMaxC) gardeningScore += 1;
  ...
  // Функция для определения цвета температуры (всегда на основе Цельсиев)
  const getTempColor = () => {
    if (tempCelsius >= tempOptimalMinC && tempCelsius <= tempOptimalMaxC) return '#4CAF50';
    else if (tempCelsius >= tempGoodMinC && tempCelsius <= tempGoodMaxC) return '#FFC107';
    else return '#FF9800';
  };

  // Функция для определения рекомендации по температуре
  const getTempRecommendation = () => {
    if (tempCelsius >= tempOptimalMinC && tempCelsius <= tempOptimalMaxC) {
      return 'Идеальная температура для работы в саду';
    } else if (tempCelsius < tempOptimalMinC) {
      if (tempCelsius < 5) {
        return 'Очень холодно - защитите растения от заморозков';
      } else {
        return 'Прохладно - защитите теплолюбивые растения';
      }
    } else {
      if (tempCelsius > 35) {
        return 'Очень жарко - поливайте растения утром и вечером';
      } else {
        return 'Жарко - поливайте растения чаще';
      }
    }
  };
  ...
  const getHumidityRecommendation = () => {
    if (humidity >= 40 && humidity <= 70) {
      return 'Оптимальная влажность для растений';
    } else if (humidity < 30) {
      return 'Очень сухой воздух - значительно увеличьте полив';
    } else if (humidity < 40) {
      return 'Сухой воздух - увеличьте полив';
    } else if (humidity > 80) {
      return 'Очень высокая влажность - усильте контроль за грибковыми заболеваниями';
    } else {
      return 'Высокая влажность - следите за грибковыми заболеваниями';
    }
  };
  ...
  const getWindRecommendation = () => {
    if (windSpeed <= windThreshold) {
      return 'Спокойные условия - отличное время для работы в саду';
    } else if (windSpeed <= windThreshold * 1.5) {
      return 'Умеренный ветер - отложите опрыскивание растений';
    } else {
      return 'Сильный ветер - отложите работы с химикатами и подвяжите высокие растения';
    }
  };
  ...
  const getAdditionalRecommendation = () => {
    if (isRain) {
      if (tempCelsius < 10) {
        return 'Дождь и холодно - лучшее время для планирования работ в помещении';
      } else {
        return 'Дождь обеспечит естественный полив';
      }
    } else {
      const isDry = humidity < 40;
      const isHot = tempCelsius > 28;
      const isWindy = windSpeed > windThreshold;
      
      if (isHot && isDry) {
        return 'Жарко и сухо - полив утром и вечером обязателен';
      } else if (isWindy && isDry) {
        return 'Ветрено и сухо - растения быстро теряют влагу';
      } else if (isHot) {
        return 'Жаркая погода - работайте в утренние или вечерние часы';
      } else {
        return 'Хорошее время для полива и прополки';
      }
    }
  };

  // Конвертируем температуру для отображения
  const tempDisplay = convertTemperature(tempCelsius, tempUnit);
  ...
  return {
    conditions: [
      ...
      },
      {
        title: 'Влажность',
        value: `${humidity}%`,
        color: (humidity >= 40 && humidity <= 70) ? '#4CAF50' : 
               (humidity >= 30 && humidity <= 80) ? '#FFC107' : 
               '#FF9800',
        icon: 'water-outline',
      },
      {
    ...
    recommendations: [
      getTempRecommendation(), // Улучшенная рекомендация по температуре
      getHumidityRecommendation(), // Детальная рекомендация по влажности
      getWindRecommendation(), // Улучшенная рекомендация по ветру
      getAdditionalRecommendation(), // Контекстуальная рекомендация
    ]
  };
};
...
const getRunningData = () => {
  ...
  // Пороговые значения всегда в Цельсиях для score
  const tempOptimalMinC = 10;
  const tempOptimalMaxC = 20;
  const tempGoodMinC = 5;
  const tempGoodMaxC = 25;
  
  if (tempCelsius >= tempOptimalMinC && tempCelsius <= tempOptimalMaxC) runningScore += 2;
  else if (tempCelsius >= tempGoodMinC && tempCelsius <= tempGoodMaxC) runningScore += 1;
  
  if (humidity <= 60) runningScore += 2;
  else if (humidity <= 75) runningScore += 1;
  
  // Адаптивные пороговые значения для ветра
  let windThreshold, windStrongThreshold;
  switch (windUnit) {
    case 'm/s':
      windThreshold = 3;
      windStrongThreshold = 6;
      break;
    case 'km/h':
      windThreshold = 10.8; // 3 м/с = 10.8 км/ч
      windStrongThreshold = 21.6; // 6 м/с = 21.6 км/ч
      break;
    case 'mph':
      windThreshold = 6.7; // 3 м/с = 6.7 mph
      windStrongThreshold = 13.4; // 6 м/с = 13.4 mph
      break;
    default:
      windThreshold = 3;
      windStrongThreshold = 6;
  ...
  if (!isRain && !isSnow) runningScore += 2;
  ...
  const getTempRecommendation = () => {
    if (tempCelsius >= tempOptimalMinC && tempCelsius <= tempOptimalMaxC) {
      return 'Идеальная температура для бега';
    } else if (tempCelsius < tempOptimalMinC) {
      if (tempCelsius < 0) {
        return 'Заморозки - будьте крайне осторожны, используйте шипы';
      } else if (tempCelsius < 5) {
        return 'Очень холодно - одевайтесь слоями, разминка обязательна';
      } else {
        return 'Прохладно - одевайтесь теплее, но не переусердствуйте';
      }
    } else {
      if (tempCelsius > 30) {
        return 'Очень жарко - бегайте только рано утром или поздно вечером';
      } else {
        return 'Жарко - бегайте рано утром или вечером, больше пейте';
      }
    }
  };
  ...
  const getHumidityRecommendation = () => {
    if (humidity <= 50) {
      return 'Отличная влажность для бега';
    } else if (humidity <= 60) {
      return 'Комфортная влажность';
    } else if (humidity <= 75) {
      return 'Повышенная влажность - больше пейте воды и снизьте темп';
    } else if (humidity <= 85) {
      return 'Высокая влажность - сократите интенсивность тренировки';
    } else {
      return 'Очень высокая влажность - рассмотрите тренировку в помещении';
    }
  };
  ...
  const getWindRecommendation = () => {
    if (windSpeed <= windThreshold) {
      return 'Спокойные условия - отлично для бега';
    } else if (windSpeed <= windStrongThreshold) {
      return 'Умеренный ветер - может помочь при попутном, мешать при встречном';
    } else {
      return 'Сильный ветер - будьте осторожны, избегайте открытых пространств';
    }
  };

  // Функция для определения рекомендации по погоде
  const getWeatherRecommendation = () => {
    if (isSnow) {
      return 'Снег - очень скользко, используйте специальную обувь';
    } else if (isRain) {
      return 'Дождь - лучше заниматься в помещении или под навесом';
    } else {
      const isHot = tempCelsius > 25;
      const isHumid = humidity > 75;
      const isWindy = windSpeed > windThreshold;
      
      if (isHot && isHumid) {
        return 'Жарко и влажно - сократите дистанцию и пейте больше воды';
      } else if (isWindy && tempCelsius < 10) {
        return 'Холодно и ветрено - защитите лицо и руки от обветривания';
      } else if (uvIndex > 5) {
        return 'Высокий UV - используйте солнцезащитный крем и кепку';
      } else {
        return 'Отличные условия для бега на свежем воздухе';
      }
    }
  };
  ...
  return {
    ...
    recommendations: [
      getTempRecommendation(), // Детальная рекомендация по температуре
      getHumidityRecommendation(), // Улучшенная рекомендация по влажности
      getWindRecommendation(), // Корректная рекомендация по ветру
      getWeatherRecommendation(), // Комплексная рекомендация по погоде
    ]
  };
};
