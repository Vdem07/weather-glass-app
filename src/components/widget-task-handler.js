...
import React from 'react';
import { AdaptiveWeatherWidget } from './widgets/AdaptiveWeatherWidget';
import { getCurrentWeather, getDailyForecast } from '../api/weather';
import AsyncStorage from '@react-native-async-storage/async-storage';
import * as Location from 'expo-location';

const nameToWidget = {
  AdaptiveWeather: AdaptiveWeatherWidget,
};
...
export async function widgetTaskHandler(props) {
  ...
  const widgetHeight = widgetInfo.height || 120;
  ...
  switch (props.widgetAction) {
    ...
    case 'WIDGET_RESIZED':
      try {
        ...
        console.log(`Widget rendered with size: ${widgetWidth}x${widgetHeight}`);
      } catch (error) {
        console.error('Error updating widget:', error);
        // Рендерим виджет с ошибкой
        props.renderWidget(
